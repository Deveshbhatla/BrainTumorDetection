...
To make only the segmented areas visible:
...
$("#predict-button").click(async function () {
	...
	var orig_image = tf.browser.fromPixels(image);
	
	// This is the color image
	var color_image = tf.browser.fromPixels(blank_image);
	...
	let input_image = tf.browser.fromPixels(image)
	.resizeNearestNeighbor([224,224]);
	
	
	// Pre-process the image
	let tensor = tf.browser.fromPixels(image)
	 .resizeBilinear([224,224]) // change the image size here
	 .toFloat()
	 .div(tf.scalar(255.0))
	.expandDims();
	...
	var preds = Array.from(predictions); // JS Code with js array
		let predicted_result=document.getElementById('prediction');
		console.log(preds);
	...
	for (i = 0; i < preds.length; i++) { 
		...
		if (num < 0.5) {   // <-- Set the threshold here
			preds[i] = 0;
			predicted_result.innerText='No Tumor';

			
		} else {
			preds[i] = 255;
			predicted_result.innerText='Detected Tumor in the brain';
